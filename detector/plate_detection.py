...
## pip3.7 install opencv-contrib-python==3.4.13.47 --force-reinstall

##https://learnopencv.com/deep-learning-based-object-detection-using-yolov3-with-opencv-python-c/S
## pip3.7 install tensorflow==2.6.0
## pip uninstall keras
...
class LicensePlateDetector:
    ...
    def post_process(self, frame, outputs, conf, queue):
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                cv2.putText(frame, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 2, color, 2)
        ...
        try:
            ...
            while True:
                if cam_cleaner.last_frame is not None:
                    ...
                    self.post_process(frame, outputs, 0.5, queue)
    ...
    def detector(self, img):
        ...
        #print(self.reco_char_cnn(plates[0])[0])
        for i in range(len(plates)):
          p += self.reco_char_cnn(plates[i])[0]
        return p
    ...
    def pydetector(self, img):
        self.detect_plate(img)
        x, y, w, h = self.coordinates
        box = self.img[y:y + h, x:x + w]
        custom_cfg = r' --psm 6 --oem 3 -c tessedit_char_list=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
        return pytesseract.image_to_string(box, config=custom_cfg)
