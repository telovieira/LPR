...
class LicensePlateDetector:
    ...
    def consumer(self):
        ...
        while not self.event.is_set() or not self.queue.empty():
            ...
            print(message)
            logging.info(
                "Consumer storing message: %s (size=%d)", message, self.queue.qsize()
            )
    ...
    def post_process(self, frame, outputs, conf):
        ...
        try:
            ...
            while True:
                if cam_cleaner.last_frame is not None:
                    ...
                    self.post_process(frame, outputs, 0.5)
